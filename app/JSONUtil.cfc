...
<cfcomponent displayname="JSONUtil" output="false">
	...
	<cffunction name="QueryToStruct" returntype="any" output="false"
		...
		<cfscript>
			...
			if (ARGUMENTS.Row){
				LOCAL.FromIndex 	= ARGUMENTS.Row;								// We are only looping over one row.
				LOCAL.ToIndex 		= ARGUMENTS.Row;
			} else {
				LOCAL.FromIndex 	= 1;													// We are looping over the entire query.
			...
			for (LOCAL.RowIndex = LOCAL.FromIndex ; LOCAL.RowIndex LTE LOCAL.ToIndex ; LOCAL.RowIndex = (LOCAL.RowIndex + 1)){
				...
				ArrayAppend( LOCAL.DataArray, StructNew() );								// Create a new structure for this row.
	
				LOCAL.DataArrayIndex = ArrayLen( LOCAL.DataArray );						// Get the index of the current data array object.
	
				// Loop over the columns to set the structure values.
				for (LOCAL.ColumnIndex = 1 ; LOCAL.ColumnIndex LTE LOCAL.ColumnCount ; LOCAL.ColumnIndex = (LOCAL.ColumnIndex + 1)){
					LOCAL.ColumnName = LOCAL.Columns[ LOCAL.ColumnIndex ];		// Get the column value.
			...
			if (ARGUMENTS.Row){
				return( LOCAL.DataArray[ 1 ] );													// Return the first array item.
			} else {
				return( LOCAL.DataArray );														// Return the entire array.
			}
